...
def workflow_compute(frt: FaasitRuntime):
    ...
    with nullcontext():
        ...
        input_pattern_A = str(_input['input']['pattern_A'])
        input_pattern_B = str(_input['input']['pattern_B'])
        output_pattern = str(_input['output']['pattern'])
        M = int(_input['input']['M'])
        N = int(_input['input']['N'])

        A_blocks = [store.get(input_pattern_A.replace('%i%', str(i)), src_stage='split') for i in range(M)]
        B_blocks = [store.get(input_pattern_B.replace('%j%', str(i)), src_stage='split') for i in range(N)]
        input_end = time.time()
    ...
    with nullcontext():
        ...
        for i in range(M):
            for j in range(N):
                C_blocks[i][j] = gemm(A_blocks[i], B_blocks[j])
    ...
    with nullcontext():
        ...
        for i in range(M):
            for j in range(N):
                store.put(output_pattern.replace('%i%', str(i)).replace('%j%', str(j)), C_blocks[i][j], dest_stages=['merge'])
...
handler = create_handler(workflow_compute)
